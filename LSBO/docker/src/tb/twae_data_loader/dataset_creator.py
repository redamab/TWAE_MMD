...
def test_direct_dataset_creation():
    """Test the direct dataset creation approach."""
    print("🔍 TESTING DIRECT DATASET CREATION")
    print("=" * 60)
    
    # Import required modules
    import sys
    sys.path.append('/workspace/TWAE_AMP_Generation/src')
    from data import create_simple_pipeline
    from twae_data_loader.config import get_config
    
    try:
        # Create tokenizer and config
        tokenizer, _ = create_simple_pipeline()
        ...
        if batch_count > 0:
            print(f"\n🎉 SUCCESS! Processed {batch_count} batches successfully")
            return True
        else:
            print("\n❌ FAILED! No batches were processed")
            return False
